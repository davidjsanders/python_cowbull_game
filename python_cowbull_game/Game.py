...
class Game:
    ...
    def _initialize(self, *args, **kwargs):
        ...
        # Validate game mode
        #
        logging.debug('Game.py: Validating game mode (if present)')
        mode = kwargs.get('mode', 'normal')
        if mode is not None:
            if mode.lower() not in Game.game_modes:
                raise ValueError('Game mode must be one of: {0}'.format(Game.game_modes))
        self._mode = mode.lower()
        logging.debug('Game.py: Game mode set to {0}'.format(self._mode))

        #
        # Check if existing game
        #
        logging.debug('Game.py: Validating game object (if present)')
        game_object = kwargs.get('game_object', None)
        if game_object is None:
            logging.debug('Game.py: Game object NOT provided; creating new.')
            dw = DigitWord()
        ...
        logging.debug('Game.py: Game object created')
